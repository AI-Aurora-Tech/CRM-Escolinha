  ...
  const processNotifyItem = async (student: Student) => {
      ...
      if (phone) {
          ...
          if (notifyIsFeeCharging) {
              ...
              msg = `Olá ${student.guardian.name}, aqui é do Pitangueiras FC! ${emoji}\n\n*COMUNICADO: ${type}*\nAtleta: *${student.name}*\n\n📌 *${notifyActivity.title}*\n📅 Data: ${formatDate(notifyActivity.date)}\n`;
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                            <div className="flex-1">
                                ...
                                {a.type === 'GAME' && isFinished && (<div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                                    ...
                                    <div className="flex items-center gap-4"><div className="text-center"><span className="text-[10px] text-blue-400 block font-bold">PITANGUEIRAS FC</span><span className="text-2xl font-black text-primary-600">{a.homeScore}</span></div><span className="text-blue-300 font-bold text-lg">X</span><div className="text-center"><span className="text-[10px] text-blue-400 block font-bold">VISITANTE</span><span className="text-2xl font-black text-blue-700">{a.awayScore}</span></div></div>
                                </div>)}
                ...
                <form onSubmit={handleFinishMatchSubmit} className="space-y-6">
                    <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                      ...
                      <div className="flex items-center gap-4">
                          <div className="flex-1 text-center"><label className="block text-[10px] font-black text-primary-600 mb-1 uppercase">Pitangueiras FC</label><input type="number" min="0" className="w-20 mx-auto border-2 border-blue-200 rounded-xl p-3 text-center text-3xl font-black focus:border-primary-500 outline-none transition-all" value={newActivity.homeScore ?? ''} onChange={e => setNewActivity({...newActivity, homeScore: e.target.value === '' ? undefined : parseInt(e.target.value)})} /></div>
                          <div className="text-2xl font-light text-blue-300 pt-4">X</div>
                          <div className="flex-1 text-center"><label className="block text-[10px] font-black text-blue-400 mb-1 uppercase truncate">{newActivity.opponent || 'Visitante'}</label><input type="number" min="0" className="w-20 mx-auto border-2 border-blue-200 rounded-xl p-3 text-center text-3xl font-black focus:border-blue-400 outline-none transition-all" value={newActivity.awayScore ?? ''} onChange={e => setNewActivity({...newActivity, awayScore: e.target.value === '' ? undefined : parseInt(e.target.value)})} /></div>
                      </div>
                    </div>
                    {(newActivity.homeScore || 0) > 0 && (<div className="space-y-3"><label className="block text-xs font-black text-blue-500 uppercase tracking-wider">Artilheiros (Pitangueiras FC)</label><div className="max-h-48 overflow-y-auto pr-2 space-y-2">{Array.from({ length: Math.min(newActivity.homeScore || 0, 20) }).map((_, idx) => (<div key={idx} className="flex items-center gap-2 bg-blue-50 p-2 rounded-lg border border-blue-100"><span className="text-[10px] font-black text-primary-600 w-8">GOL {idx + 1}</span><select className="flex-1 border rounded-lg p-2 bg-white text-xs outline-none focus:ring-2 focus:ring-primary-500" value={newActivity.scorers?.[idx] || ''} onChange={e => updateScorer(idx, e.target.value)} required><option value="">Quem marcou?</option>{getAttendeesList(newActivity).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>))}</div></div>)}
